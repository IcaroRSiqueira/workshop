...
feature 'User send result to challenge' do
 ...
 scenario 'already had sent one solution' do
    challenge = create(:challenge, title: 'Desafio 01', 
                                   description: 'Quanto é 10 + 10')
    user = create(:user)
    solution = create(:solution, challenge: challenge, user: user)
    login_as user

    visit new_challenge_solution_path(challenge, solution)
    fill_in 'Resultado', with: '20'
    click_on 'Enviar'
  ...
  scenario 'already had sent one solution' do
    ...
    expect(page).to have_content('Você já respondeu esse desafio.')
  end
end